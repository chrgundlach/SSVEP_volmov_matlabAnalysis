...
F.PathOut               = 'C:\Matlab_user\Christopher\SSVEP_volmov\EEG\TFA_lagged_regression';
...
F.PathInRESS            = 'C:\Matlab_user\Christopher\SSVEP_volmov\EEG\TFA_Gabor_RESS_noblinks_CSD';
...
% F.Subjects2Use          = [14 17 18 19 20 25 26 27 24];
F.EEGChans              = 1:64;
F.EMGChans              = [71 72];
F.VEOGChans             = [];
F.HEOGChans             = [];
...
F.TFAfreqs              = [4-8*(1/3):0.25:31];
F.TFAfreqs              = [4-7*(1/3)-0.5:0.25:31];
...
for i_sub = 1:numel(F.Subjects2Use)
    ...
    %% loop across each frequency 1:09:61
    ...
    for i_freq = 1:numel(TFA.source_freq)      
        ...
        if i_freq == 1
            ...
            REG_data_b = nan(EEG_noress_ep_gab.nbchan,numel(TFA.source_freq),numel( REG.time));
            REG_data_p = nan(EEG_noress_ep_gab.nbchan,numel(TFA.source_freq),numel( REG.time));
            REG_data_tstat = nan(EEG_noress_ep_gab.nbchan,numel(TFA.source_freq),numel( REG.time));
            REG_data_Rsqr_adj = nan(EEG_noress_ep_gab.nbchan,numel(TFA.source_freq),numel( REG.time));
            % frequencies
        ...
        for i_lag = 1:numel(REG.pntindex)
            ...
            t_data_regressor = reshape(TFA.regressor_data(t.ind_regressor,:),[],1);
            t_data_source = reshape(t.source_data(:,t.ind_source,:),size(t.source_data,1),[]);
                    
            % graphical checking
            %figure; scatter(t.data_regressor,t_data_source(29,:),'.')
            %figure; scatter(zscore(t.data_regressor),zscore(t_data_source(29,:)),'.')
            ...
            %    regress(t.data_regressor,[ones(numel(t.data_regressor),1) t_data_source(29,:)']);
            
            % loop across electrodes
            
            %% troubleshooting
            
%             parfor i_el = 1:size(t_data_source,1)
%                 t_nanindex{i_el} = ~(isnan(t_data_source(i_el,:)') | isnan(t_data_regressor));
%                 regr{i_el} = fitlm(t_data_source(i_el,t_nanindex{i_el} ),t_data_regressor(t_nanindex{i_el} ),'linear');
%                 REG_data_b(i_el,i_freq,i_lag) = regr{i_el}.Coefficients.Estimate(2);
%             end
            
            clear regr t_nanindex
%             parfor i_el = 1:size(t_data_source,1)
            for i_el = 1:size(t_data_source,1)
                %% loop across electrodes
                ...
                % linear regeression
                regr{i_el} = fitlm(t_data_source(i_el,t_nanindex{i_el} ),t_data_regressor(t_nanindex{i_el} ),'linear');
%                 figure; subplot(plot(t_data_source(i_el,:))); figure; plot(t_data_regressor)
                %figure; scatter(t.data_regressor(t.nanindex ),t_data_source(i_el,t.nanindex ),'.')
                %figure; scatter(zscore(t.data_regressor(t.nanindex )),zscore(t_data_source(i_el,t.nanindex )),'.')
                
                % get estimate for electrode, frequence, lag
                REG_data_b(i_el,i_freq,i_lag) = regr{i_el}.Coefficients.Estimate(2);
                
                % get tstat for electrode, frequence, lag
                REG_data_tstat(i_el,i_freq,i_lag) = regr{i_el}.Coefficients.tStat(2);
                
                % get p-value for electrode, frequence, lag
                REG_data_p(i_el,i_freq,i_lag) = regr{i_el}.Coefficients.pValue(2);
                
                % get R-squared for electrode, frequence, lag
                REG_data_Rsqr_adj(i_el,i_freq,i_lag) = regr{i_el}.Rsquared.Adjusted;
...
%         figure; plot(REG.time,squeeze(REG.data_tstat(:,i_freq,:))');  title('t-stat')
        ...
        REG.data_b = REG_data_b;
        REG.data_p = REG_data_p;
        REG.data_tstat = REG_data_tstat;
        REG.data_Rsqr_adj = REG_data_Rsqr_adj;
    ...
    fprintf(1,'|| saving file ||  %s\\VP%02.0f_exp_lagreg_noblinks_v2_detrended.mat ||\n', F.PathOut,F.Subjects2Use(i_sub)')
    if ~exist(F.PathOut); mkdir(F.PathOut); end
    save(sprintf('%s\\VP%02.0f_exp_lagreg_withSSVEP_noblinks_v2_detrended.mat', F.PathOut,F.Subjects2Use(i_sub)), 'REG')   
