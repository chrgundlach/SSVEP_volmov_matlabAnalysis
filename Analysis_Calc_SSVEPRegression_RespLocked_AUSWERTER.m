...
F.PathOut               = 'C:\Matlab_user\Christopher\SSVEP_volmov\EEG\TFA_lagged_regression_resplocked';
...
F.Subjects2Use          = [5 12 20 27];
% F.Subjects2Use          = [ 5 7 8 ];
% F.Subjects2Use          = [ 17 18];
% F.Subjects2Use          = [ 26 27];
F.EEGChans              = 1:64;
F.EMGChans              = [71 72];
F.VEOGChans             = [];
F.HEOGChans             = [];
...
for i_sub = 1:numel(F.Subjects2Use)
    ...
    for i_freq = 1:numel(TFA.regressor_freq)
        ...
        %%%%%%%%%%%
        % detrend
        EEG_ress_ep_gab = eegF_Detrend(EEG_ress_ep_gab,[0.5 EEG_ress_ep_gab.times(end)/1000-0.5]);
        %%%%%%%%%%%
        
        %%%%%%%%%% remove data around blink
        t.blink_win_idx = squeeze(true(size(EEG_ress_ep_gab.data))); % index for blinks
        for i_tr = 1:EEG_ress_ep_gab.trials
            ...
            for i_blink = 1:numel(t.blinksamples)
                ...
                EEG_ress_ep_gab.data(1,t.blinkwindow_i2,i_tr) = nan;
                % index blinks
                t.blink_win_idx(t.blinkwindow_i2,i_tr) = false;
            end
        end
        t.blinfreedata = EEG_ress_ep_gab.data;
        % pop_eegplot(EEG_ress_ep_gab,1,1,1,1)
        %figure; plot(EEG_ress_ep_gab.times,squeeze(EEG_ress_ep_gab.data))
        %figure; plot(EEG_ress_ep_gab.times,t.blink_win_idx)
        %%%%%%%%%%%
        ...
        t.resp_win_idx = squeeze(false(size(EEG_ress_ep_gab.data)));
        t.resp_time_mat = nan(EEG_ress_ep_gab.pnts,EEG_ress_ep_gab.trials);
        for i_tr = 1:EEG_ress_ep_gab.trials
            % when was the button pressed?
            t.respsamples = cell2mat(EEG_ress_ep_gab.epoch(i_tr).eventlatency(...
                cell2mat([EEG_ress_ep_gab.epoch(i_tr).eventtype])==str2num(F.RespTrigger{1})))/1000*EEG_ress_ep_gab.srate;
            % loop across button presses to replace data not clos to button presses by nan
            t.respwindow_i = F.RespWindow./1000*EEG_ress_ep_gab.srate;
            ...
            for i_resp = 1:numel(t.respsamples)
                if t.respsamples(i_resp)+t.respwindow_i(1)>0 & t.respsamples(i_resp)+t.respwindow_i(2) <= EEG_ress_ep_gab.pnts
                    % index values closest to blink.window (even if blink is close to start end)
                    t.respwindow_i2 = dsearchn((1:EEG_ress_ep_gab.pnts)', (t.respsamples(i_resp)+t.respwindow_i(1))):...
                        dsearchn((1:EEG_ress_ep_gab.pnts)', (t.respsamples(i_resp)+floor(t.respwindow_i(2))));
                    % replace values around blink with nans
        ...
        EEG_ress_ep_gab.data(~t.resp_win_idx)=nan;
        % pop_eegplot(EEG_ress_ep_gab,1,1,1,1)
        %%%%%%%%%%%
        ...
        t.data(:,:,i_freq) = imresize(squeeze(mean(EEG_ress_ep_gab.data,1)),[floor(EEG_ress_ep_gab.pnts/TFA.resize) EEG_ress_ep_gab.trials]);
