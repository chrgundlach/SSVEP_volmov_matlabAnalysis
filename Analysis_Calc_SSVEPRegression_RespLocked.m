...
F.RespWindow            = [-3000 2999]; % time window in ms around responses which will be considered
            ...
            for i_resp = 1:numel(t.respsamples)
                if t.respsamples(i_resp)+t.respwindow_i(1)>0 & t.respsamples(i_resp)+t.respwindow_i(2) <= EEG_ress_ep_gab.pnts
                    ...
                    t.respwindow_i2 = dsearchn((1:EEG_ress_ep_gab.pnts)', (t.respsamples(i_resp)+t.respwindow_i(1))):...
                        dsearchn((1:EEG_ress_ep_gab.pnts)', (t.respsamples(i_resp)+t.respwindow_i(2)));
                    % replace values around blink with nans
...
%                 %                 subplot(3,1,3); plot( REG.data_tstat(:,i_freq,i_lag)); title('t-stat')
                ...
                subplot(2,2,2); plot(t.allresptimes_res,squeeze(log10(REG.data_p(i_el,i_freq,i_lag,:)))'); hline(log10(.05),'r'); title('p-vals');
