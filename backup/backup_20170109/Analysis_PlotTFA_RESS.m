...
clear all
%% parameters
F.PathIn                = 'F:\work\data\SSVEP_volmov\EEG\TFA_SCADS_Gabor_RESS';
% F.Subjects2Use          = [1];
F.Subjects2Use          = [1:20];
...
for i_sub=1:numel(F.Subjects2Use)
    ...
    if i_sub == 1
        ...
        TFA.data_RESS_induced = nan([size(temp.tfa.TFA.data_RESS_induced) numel(F.Subjects2Use)]);
        ...
        TFA.RESS_map = nan([length(temp.tfa.TFA.RESSmap) numel(F.Subjects2Use)]);
...
%         mean(temp.tfa.TFA.data_RESS_evoked(:,eeg_time2points(TFA.baseline(1),TFA.time):eeg_time2points(TFA.baseline(2),TFA.time),:,:),2)))-1);
    ...
    TFA.RESS_map(:,i_sub) = temp.tfa.TFA.RESSmap(:,end);
    TFA.RESS_snr_ind(i_sub)=temp.tfa.TFA.RESSSNR_ind(end);
    TFA.RESS_snr_evo(i_sub)=temp.tfa.TFA.RESSSNR_evo(end);
...
set(gca,'YDir','normal')
title(sprintf('tfa noRESS, for channel %s;\n averaged in time domain', vararg2str(pl.elec2plot)), 'FontSize',8)
colorbar
xlabel('time in ms')
ylabel('frequency in Hz')
xlim(pl.xlims)
...
set(gca,'YDir','normal')
title(sprintf('p-values noRESS, for channel %s;\n averaged in frequency domain; baseline [%1.0f %1.0f]ms',...
    vararg2str(pl.elec2plot),TFA.baseline),'FontSize',8)
freezeColors
h.cb2=colorbar;
xlabel('time in ms')
...
imagesc(TFA.time,TFA.frequency,pl.data_evo,t.clims2)
set(gca,'YDir','normal')
title(sprintf('normalized tfa noRESS, for channel %s;\n averaged in time domain; baseline [%1.0f %1.0f]ms',...
    vararg2str(pl.elec2plot),TFA.baseline),'FontSize',8)
freezeColors
...
imagesc(TFA.time,TFA.frequency,pl.data_ind,t.clims1)
set(gca,'YDir','normal')
title(sprintf('tfa RESS component; averaged in frequency domain'),'FontSize',8)
colorbar
xlabel('time in ms')
ylabel('frequency in Hz')
xlim(pl.xlims)
...
imagesc(TFA.time,TFA.frequency,pl.data_evo,t.clims2)
set(gca,'YDir','normal')
title(sprintf('tfa RESS component; averaged in time domain'),'FontSize',8)
colorbar
xlabel('time in ms')
ylabel('frequency in Hz')
xlim(pl.xlims)
...
h.sp2=subplot(2,1,2);
for i_freq = 1:numel(TFA.frequency)
    [tt.h tt.p tt.ci tt.stats]=ttest(squeeze(TFA.data_RESS_induced_bc(i_freq,:,:))');
...
h.sp2=subplot(2,1,2);
for i_freq = 1:numel(TFA.frequency)
    [tt.h tt.p tt.ci tt.stats]=ttest(squeeze(TFA.data_RESS_evoked_bc(i_freq,:,:))');
...
%     };
pl.parameters = {...
    [14.16667-3.25 14.16667+3.25] {'RESS'} 'data_RESS_induced_bc' [0.6 0.6 0.6] 2 sprintf('vis ind\nSSVEP') 'RESS';...
    [14.16667-3.25 14.16667+3.25] {'RESS'} 'data_RESS_evoked_bc' [0 0 0] 2 sprintf('vis evo\nSSVEP') 'RESS';...
