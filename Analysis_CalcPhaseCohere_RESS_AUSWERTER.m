...
F.PathOut               = 'C:\Matlab_user\Christopher\SSVEP_volmov\EEG\PHASECOHERE_RESS_noblinks_CSD';
...
F.PathInRESS            = 'C:\Matlab_user\Christopher\SSVEP_volmov\EEG\TFA_Gabor_RESS_noblinks_CSD4'; % gabor of 1Hz FWHM [no blinks -3.5 3.5]
...
%     EEG = pop_resample(EEG, 256);
    ...
    if F.CSD_flag == 1 & i_sub == 1 % calculate CSD matrix
        CSD.chanmat=ExtractMontage('C:\Users\EEG\Documents\MATLAB\christopher\general_functions\CSD\resource\10-5-System_Mastoids_EGI129.csd',{EEG_exp.chanlocs.labels}');
...
%     sinf.sin.f = 85/6+7;                          % frequency
    sinf.filt_range = 5;
    sinf.time2an = [-3250 3250];
    %sinf.filter = [-1 1].*sinf.filt_range+sinf.sin.f;
    sinf.filter = [0.1 100];
    sinf.filt_order = 32*EEG_expep_sh_ress.srate; %32*EEG_expep_sh_ress.srate
    EEG_expep_sh_ress_cut = pop_select(EEG_expep_sh_ress,'time',sinf.time2an./1000);
    EEG_expep_sh_chan_cut = pop_select(EEG_expep_sh_chan,'time',sinf.time2an./1000);
    EEG_expep_sh_chan = pop_select(EEG_expep_sh,'channel',t.chan.StandardLabel);
    EEG_expep_sh_ress_f = pop_eegfiltnew(EEG_expep_sh_ress,sinf.filter(1),sinf.filter(2), sinf.filt_order, 0, [], 0); %32*EEG_expep_sh_ress.srate
    EEG_expep_sh_chan_f = pop_eegfiltnew(EEG_expep_sh_chan,sinf.filter(1),sinf.filter(2), sinf.filt_order, 0, [], 0);
    EEG_expep_sh_ress_f = pop_select(EEG_expep_sh_ress_f,'time',sinf.time2an./1000);
    EEG_expep_sh_chan_f = pop_select(EEG_expep_sh_chan_f,'time',sinf.time2an./1000);
    ...
    EEG_expep_sh_ress_m = pop_select(EEG_expep_sh_ress,'trial',1:2); EEG_expep_sh_ress_m.data(:,:,1) = mean(EEG_expep_sh_ress.data,3);
    EEG_expep_sh_chan_m = pop_select(EEG_expep_sh_chan,'trial',1:2); EEG_expep_sh_chan_m.data(:,:,1) = mean(EEG_expep_sh_chan.data,3);
    EEG_expep_sh_ress_m_f = pop_eegfiltnew(EEG_expep_sh_ress_m,sinf.filter(1),sinf.filter(2), sinf.filt_order, 0, [], 0); %32*EEG_expep_sh_ress.srate
    EEG_expep_sh_chan_m_f = pop_eegfiltnew(EEG_expep_sh_chan_m,sinf.filter(1),sinf.filter(2), sinf.filt_order, 0, [], 0);
    EEG_expep_sh_ress_m_f = pop_select(EEG_expep_sh_ress_m_f,'time',sinf.time2an./1000); EEG_expep_sh_ress_m_f = pop_select(EEG_expep_sh_ress_m_f,'trial',1);
    EEG_expep_sh_chan_m_f = pop_select(EEG_expep_sh_chan_m_f,'time',sinf.time2an./1000); EEG_expep_sh_chan_m_f = pop_select(EEG_expep_sh_chan_m_f,'trial',1);
    EEG_expep_sh_ress_m = pop_select(EEG_expep_sh_ress_m,'time',sinf.time2an./1000); EEG_expep_sh_ress_m = pop_select(EEG_expep_sh_ress_m,'trial',1);
    EEG_expep_sh_chan_m = pop_select(EEG_expep_sh_chan_m,'time',sinf.time2an./1000); EEG_expep_sh_chan_m = pop_select(EEG_expep_sh_chan_m,'trial',1);
     
%     figure; subplot(2,1,1); plot(EEG_expep_sh_ress_cut.times,[EEG_expep_sh_ress_cut.data(1,:,1);EEG_expep_sh_ress_f.data(1,:,1); EEG_expep_sh_ress_m.data(1,:,1); EEG_expep_sh_ress_m_f.data(1,:,1)])
%     legend({'raw 1trial';'filt 1trial'; 'raw mean'; 'filt mean'}); title('RESS')
%     subplot(2,1,2); plot(EEG_expep_sh_chan_cut.times,[EEG_expep_sh_chan_cut.data(1,:,1);EEG_expep_sh_chan_f.data(1,:,1); EEG_expep_sh_chan_m.data(1,:,1); EEG_expep_sh_chan_m_f.data(1,:,1)])
%     legend({'raw 1trial';'filt 1trial'; 'raw mean'; 'filt mean'}); title(t.chan.StandardLabel{1})
    
%     figure; subplot(2,1,1); plot(EEG_expep_sh_ress_cut.times,[EEG_expep_sh_ress_f_m.data(1,:,1);EEG_expep_sh_ress_m_f.data(1,:,1); EEG_expep_sh_ress_f_m.data(1,:,1)-EEG_expep_sh_ress_m_f.data(1,:,1)])
%     legend({'filt mean';'mean filt';'diff'}); title('RESS')
%     subplot(2,1,2); plot(EEG_expep_sh_ress_cut.times,[EEG_expep_sh_chan_f_m.data(1,:,1);EEG_expep_sh_chan_m_f.data(1,:,1); EEG_expep_sh_chan_f_m.data(1,:,1)-EEG_expep_sh_chan_m_f.data(1,:,1)])
...
%     figure; histogram(EEG_expep_sh_chan_f_m.data(1,:,1)-EEG_expep_sh_chan_m_f.data(1,:,1),1000)
    ...
    sinf.sin.cycle_pnts = ceil(EEG_expep_sh_ress_f_m.srate/sinf.sin.f); % number of points for one cycle
    sinf.baseline = [-3250 -2750];              % baseline window
    sinf.baseline_i = dsearchn(EEG_expep_sh_ress_f_m.times',sinf.baseline');
    sinf.sin.T = 1/EEG_expep_sh_ress_f_m.srate;  % sampling period
%     sinf.sin.L = diff(sinf.baseline_i);         % length of signal
    sinf.sin.L = EEG_expep_sh_ress_f_m.pnts + sinf.sin.cycle_pnts;         % length of signal
    sinf.sin.t =  (0:sinf.sin.L-1)*sinf.sin.T;  % Time vector
    EEG_expep_sh_ress_m_f.data = detrend(EEG_expep_sh_ress_m_f.data);
    EEG_expep_sh_chan_m_f.data = detrend(EEG_expep_sh_chan_m_f.data);
    sinf.sin.amp_ress_m = (max(EEG_expep_sh_ress_f_m.data) - min(EEG_expep_sh_ress_f_m.data))/2;
    sinf.sin.amp_chan_m = (max(EEG_expep_sh_chan_f_m.data) - min(EEG_expep_sh_chan_f_m.data))/2;
    ...
    sinf.bin_width_pnt = ceil(sinf.bin_width_t/1000*EEG_expep_sh_ress_f_m.srate);
    
%     figure; subplot(2,1,1); plot(EEG_expep_sh_ress_cut.times,EEG_expep_sh_ress_m_f.data(:,:,1)); hold on;
%     plot(EEG_expep_sh_ress_cut.times(sinf.baseline_i(1):sinf.baseline_i(2)-1),sinf.sin.signal_ress_m(sinf.baseline_i(1):sinf.baseline_i(2)-1));
%     subplot(2,1,2); plot(EEG_expep_sh_chan_cut.times,EEG_expep_sh_chan_m_f.data(:,:,1)); hold on;
%     plot(EEG_expep_sh_chan_cut.times(sinf.baseline_i(1):sinf.baseline_i(2)-1),sinf.sin.signal_chan_m(sinf.baseline_i(1):sinf.baseline_i(2)-1));
%     
    ...
    for i_shift = 1:sinf.sin.cycle_pnts
        ...
        % figure; plot([sinf.sin.signal_ress_m_sh(sinf.baseline_i(1):sinf.baseline_i(2)-1); EEG_expep_sh_ress_m_f.data(:,sinf.baseline_i(1):sinf.baseline_i(2)-1,1)]')
...
%             figure; plot([t.signal_ress_st_sh(sinf.baseline_i(1):sinf.baseline_i(2)-1); ...
%             EEG_expep_sh_ress_f.data(:,sinf.baseline_i(1):sinf.baseline_i(2)-1,i_trial)]')
...
%     figure;  plot(EEG_expep_sh_ress_f_m.times,EEG_expep_sh_ress_f_m.data(:,:,1)); hold on;
%     plot(EEG_expep_sh_ress_f_m.times,sinf.sin.signal_ress_m_sh);
...
%     figure; histogram([sinf.sin.bsl_shift_ress_st-sinf.sin.bsl_shift_ress_m]); hold on; histogram(sinf.sin.bsl_shift_chan_st-sinf.sin.bsl_shift_chan_m)
    ...
    for i_time = 1:numel(sinf.sin.signal_ress_m_sh)
        ...
        % figure; plot(t.data_ress_signal_H); hold on; plot(t.data_ress_sin_H);
        % loop across shifts
...
%         
        ...
        % checking
%         t.sin_signal_ress_sh_sig = sinf.sin.amp_ress_m*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_ress_sig==min(t.sumofsq_ress_sig))-1)*sinf.sin.T)));
%         t.sin_signal_ress_sh_sin = sinf.sin.amp_ress_m*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_ress_sin==min(t.sumofsq_ress_sin))-1)*sinf.sin.T)));
%         t.sin_signal_chan_sh_sig = sinf.sin.amp_chan_m*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_chan_sig==min(t.sumofsq_chan_sig))-1)*sinf.sin.T)));
%         t.sin_signal_chan_sh_sin = sinf.sin.amp_chan_m*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_chan_sin==min(t.sumofsq_chan_sin))-1)*sinf.sin.T)));
%         t.sin_signal_ress_sh_sig_H = t.sin_signal_ress_sh_sig.*hann(numel(t.sin_signal_ress_sh_sig))';
%         t.sin_signal_ress_sh_sin_H = t.sin_signal_ress_sh_sin.*hann(numel(t.sin_signal_ress_sh_sin))';
%         t.sin_signal_chan_sh_sig_H = t.sin_signal_chan_sh_sig.*hann(numel(t.sin_signal_chan_sh_sig))';
%         t.sin_signal_chan_sh_sin_H = t.sin_signal_chan_sh_sin.*hann(numel(t.sin_signal_chan_sh_sin))';
%         
%         figure; plot(0:numel(t.win)-1,t.data_ress_signal_H); hold on; plot(0:numel(t.win)-1,t.sin_signal_ress_sh_sig_H); plot(0:numel(t.win)-1,t.sin_signal_ress_sh_sin_H);
%         figure; plot(0:numel(t.win)-1,t.data_chan_signal_H); hold on; plot(0:numel(t.win)-1,t.sin_signal_chan_sh_sig_H); plot(0:numel(t.win)-1,t.sin_signal_chan_sh_sin_H);
    end
%     figure; plot(EEG_expep_sh_ress_cut.times,sinf.lag_RESS_m); hold on;  plot(EEG_expep_sh_ress_cut.times,sinf.lag_chan_m)
...
%             % checking
%             t.sin_signal_ress_sh_sig = sinf.sin.amp_ress_st(i_trial)*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_ress_sig==min(t.sumofsq_ress_sig))-1)*sinf.sin.T)));
%             t.sin_signal_ress_sh_sin = sinf.sin.amp_ress_st(i_trial)*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_ress_sin==min(t.sumofsq_ress_sin))-1)*sinf.sin.T)));
%             t.sin_signal_chan_sh_sig = sinf.sin.amp_chan_st(i_trial)*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_chan_sig==min(t.sumofsq_chan_sig))-1)*sinf.sin.T)));
%             t.sin_signal_chan_sh_sin = sinf.sin.amp_chan_st(i_trial)*sin(2*pi*sinf.sin.f*((0:numel(t.win)-1)*sinf.sin.T+(( find(t.sumofsq_chan_sin==min(t.sumofsq_chan_sin))-1)*sinf.sin.T)));
%             t.sin_signal_ress_sh_sig_H = t.sin_signal_ress_sh_sig.*hann(numel(t.sin_signal_ress_sh_sig))';
%             t.sin_signal_ress_sh_sin_H = t.sin_signal_ress_sh_sin.*hann(numel(t.sin_signal_ress_sh_sin))';
%             t.sin_signal_chan_sh_sig_H = t.sin_signal_chan_sh_sig.*hann(numel(t.sin_signal_chan_sh_sig))';
%             t.sin_signal_chan_sh_sin_H = t.sin_signal_chan_sh_sin.*hann(numel(t.sin_signal_chan_sh_sin))';
...
%             figure; plot(0:numel(t.win)-1,t.data_chan_signal_H); hold on; plot(0:numel(t.win)-1,t.sin_signal_chan_sh_sig_H); plot(0:numel(t.win)-1,t.sin_signal_chan_sh_sin_H);
    ...
    close(h_wtb)
    figure; subplot(2,1,1); plot(EEG_expep_sh_ress_f.times,sinf.lag_RESS(1:EEG_expep_sh_ress_f.pnts,:));
    subplot(2,1,2); plot(EEG_expep_sh_chan_f.times,sinf.lag_RESS(1:EEG_expep_sh_chan_f.pnts,:));
